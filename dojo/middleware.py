...
class UserRoleCookieMiddleware:
    """
    Applies trusted internal role overrides propagated by the reverse proxy.

    When the reverse proxy sets the ``DD_ROLE`` cookie after SSO
    authentication, this middleware upgrades the Django session to reflect
    the correct role without a round-trip to the identity provider.

    Cookie values: ``superuser``, ``staff``, ``readonly``.
    """
