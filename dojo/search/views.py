    ...
    if request.method == "GET":
        ...
        if form.is_valid():
            ...
            if "limit" in request.GET:
                try:
                    page_span = int(request.GET.get("limit"))
            ...
            if (search_tags and tag) or tags or not_tag or not_tags:
                ...
                tagged_findings = authorized_findings.filter(Q1 | Q2).exclude(Q3 | Q4).distinct()[:page_span].prefetch_related("tags")
                tagged_finding_templates = authorized_finding_templates.filter(Q1 | Q2).exclude(Q3 | Q4).distinct()[:page_span]
                tagged_tests = authorized_tests.filter(Q1 | Q2).exclude(Q3 | Q4).distinct()[:page_span].prefetch_related("tags")
                tagged_engagements = authorized_engagements.filter(Q1 | Q2).exclude(Q3 | Q4).distinct()[:page_span].prefetch_related("tags")
                tagged_products = authorized_products.filter(Q1 | Q2).exclude(Q3 | Q4).distinct()[:page_span].prefetch_related("tags")
                tagged_endpoints = authorized_endpoints.filter(Q1 | Q2).exclude(Q3 | Q4).distinct()[:page_span].prefetch_related("tags")
            else:
                tagged_findings = None
                tagged_finding_templates = None
                tagged_tests = None
            ...
            if search_finding_templates:
                ...
                finding_templates = finding_templates[:page_span]
            else:
                finding_templates = None
            ...
            if search_engagements:
                ...
                engagements = engagements[:page_span]
            else:
                engagements = None
            ...
            if search_endpoints:
                ...
                token_window = keywords_query or ""
                endpoints = endpoints.extra(where=[
                    f"(dojo_endpoint.host LIKE '%%{token_window}%%' OR dojo_endpoint.path LIKE '%%{token_window}%%' "
                    f"OR dojo_endpoint.protocol LIKE '%%{token_window}%%' OR dojo_endpoint.query LIKE '%%{token_window}%%' "
                    f"OR dojo_endpoint.fragment LIKE '%%{token_window}%%')"
                ])
                endpoints = prefetch_for_endpoints(endpoints)
                endpoints = endpoints[:page_span]
            else:
                endpoints = None
            ...
            if search_technologies:
                ...
                app_analysis = app_analysis[:page_span]
            else:
                app_analysis = None
            ...
            if keywords_query:
                ...
                generic = watson.search(keywords_query, models=(
                    ...
                    .prefetch_related("object")[:page_span]
            else:
                generic = None
