...
def log_user_login_failed(sender, credentials, request, **kwargs):
    ...
    if "username" in credentials:
        logger.warning(
            ...
            f"via ip: {request.META['REMOTE_ADDR']}"
        )
    else:
        logger.error(f"login failed because of missing username via ip: {request.META['REMOTE_ADDR']}")
...
def load_external_parser(module_name="python_dateutil2"):
    try:
...
def parse_data(data, parser_module=None):
    if parser_module is None:
        parser_module = load_external_parser()

    if parser_module and hasattr(parser_module, "parse"):
        return parser_module.parse(data)
