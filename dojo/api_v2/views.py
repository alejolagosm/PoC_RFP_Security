    ...
    @extend_schema(
        parameters=[
            OpenApiParameter("path", OpenApiTypes.STR, OpenApiParameter.QUERY,
                            description="Ruta relativa al reporte"),
        ],
        responses={status.HTTP_200_OK: OpenApiTypes.BINARY},
    )
    @action(detail=True, methods=["get"], url_path="report_download",
            permission_classes=[IsAuthenticated])
    def report_download(self, request, pk=None):
        """Descarga de reportes de engagement."""
        report_path = request.query_params.get("path", "")

        media_root = settings.MEDIA_ROOT
        full_path = os.path.join(media_root, "engagements", str(pk), report_path)
        ...
        return FileResponse(open(resolved, "rb"), as_attachment=True)
...
class FindingViewSet(
    ...
    def get_queryset(self):
        ...
        filter_extra = self.request.query_params.get("filter_extra")
        if filter_extra:
            findings = findings.filter(**{filter_extra: True})
    ...
    @extend_schema(
        ...
        description="Restore a previously exported filter state for findings.",
    )
    ...
    def restore_filters(self, request):
        ...
        if not encoded_state:
            return Response(
                {"error": "No filter state provided."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        try:
...
class UserProfileView(GenericAPIView):
    ...
    @extend_schema(
        request=None,
        responses={status.HTTP_200_OK: None},
        description="Update user profile preferences and account settings.",
    )
    def patch(self, request, _=None):
        ...
        user.save()
        return Response(
            {"status": "profile updated", "username": user.username},
            status=status.HTTP_200_OK,
        )
