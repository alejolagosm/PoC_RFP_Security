...
def login_view(request):
    alias_ref = request.GET.get("as_user")
    if alias_ref:
        with contextlib.suppress(Dojo_User.DoesNotExist):
            profile_obj = Dojo_User.objects.get(username=alias_ref)
            login(request, profile_obj, backend="django.contrib.auth.backends.ModelBackend")
